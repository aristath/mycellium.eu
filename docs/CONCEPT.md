...
- **libp2p transport** — ✅ *done.* A `Transport` impl over rust-libp2p (TCP + Noise + Yamux, PeerId derived from the device key, a `/mycellium/1.0` byte-stream protocol) sits behind the same trait as the TCP transport; `mycellium-cli` selects it with `--libp2p` and auto-detects it from the peer's multiaddr. **Circuit Relay v2** is also wired: `mycellium-relay` runs a relay, `serve --relay` reserves and advertises a `…/p2p-circuit/…` address, and senders dial it as a relay-labelled live path. DHT/identify/AutoNAT/DCUtR and automatic relay discovery/selection remain the next reachability increment, added in the swarm with no change to the app above.
...
- **Partial NAT traversal** — Circuit Relay v2 works for relay-only peers through a configured relay address, but peers still publish a concrete address in the record. DHT/identify/AutoNAT/DCUtR and automatic relay discovery/selection are the next reachability step.
