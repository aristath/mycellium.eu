...
impl Identity {
    ...
    fn derive(mnemonic: bip39::Mnemonic) -> Result<Self, Error> {
        ...
        // Device and messaging keys are Messe-internal (no external HD standard
        // for X25519), so they stay on domain-separated HKDF from the same seed.
        let device = DeviceSigningKey::from_bytes(&derive_key(&seed, b"messe:device:ed25519:v1"));
        let messaging = StaticSecret::from(derive_key(&seed, b"messe:messaging:x25519:v1"));
        let signed_pre_key = StaticSecret::from(derive_key(&seed, b"messe:spk:x25519:v1:0"));
        seed.zeroize();
        Ok(Self {
    ...
    pub fn storage_key(&self) -> [u8; 32] {
        ...
        let mut key = [0u8; 32];
        hk.expand(b"messe:local-storage:v1", &mut key)
...
///
/// Used for the Messe-internal device and messaging keys. The wallet key uses
