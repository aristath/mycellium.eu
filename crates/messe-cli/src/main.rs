//! `messe` — the Full-tier client shell.
//!
...
use messe_core::group::{Group, GroupMessage};
use messe_core::identity::{Handle, Identity, MessagingPublicKey, PeerId};
use messe_core::message::{AppMessage, Body};
use messe_core::offline::Envelope;
use messe_core::platform::Platform;
use messe_core::ratchet::{Ratchet, RatchetMessage};
use messe_core::record::{Record, SignedPreKey, SignedRecord};
use messe_core::safety;
use messe_core::shamir::{self, Share};
use messe_core::transport::Transport;
use messe_core::wire;
use messe_core::x3dh::{self, HandshakeInit};
...
#[derive(Parser)]
#[command(name = "messe", about = "Messe peer-to-peer messenger (POC client)")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    /// Import a backup into a fresh MESSE_HOME.
    Import {
        /// Backup file path.
        path: String,
    },
...
fn import_backup(path: &str) -> Result<()> {
    if store::exists() {
        bail!(
            "an identity already exists at {} — import into a fresh MESSE_HOME",
            store::path().display()
        );
    }
...
/// Save an attachment to `MESSE_HOME/downloads` (name sanitized to a basename).
