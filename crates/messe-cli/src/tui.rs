...
use messe_core::message::AppMessage;
use messe_core::platform::Platform;
use messe_core::ratchet::RatchetMessage;
use messe_core::wire;
...
fn submit(
    ratchet: &Arc<Mutex<messe_core::ratchet::Ratchet>>,
    ad: &[u8],
    ...
    let app = AppMessage {
        ...
        expires_at: None,
        body: messe_core::message::Body::Text(input.to_string()),
    };
...
fn render(frame: &mut Frame, model: &ChatModel, input: &str) {
    ...
    frame.render_widget(
        Paragraph::new(transcript).block(
            Block::default()
                .borders(Borders::ALL)
                .title(format!("messe — chatting with {}", model.peer_name)),
        ),
        chunks[0],
    );
    frame.render_widget(
