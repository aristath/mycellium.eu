...
//! (Layer 8.1), and a `/messe/1.0` byte-stream protocol carrying the same
...
const PROTOCOL: StreamProtocol = StreamProtocol::new("/messe/1.0");
...
impl Libp2pNode {
    ...
    /// Wait for an inbound `/messe/1.0` stream and return it as a connection.
