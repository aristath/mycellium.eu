...
fn start() -> String {
    ...
    std::thread::spawn(move || {
        let _ = messe_directory::serve(&serve);
    });
...
fn mailbox_without_auth_is_401() {
    let base = start();
    assert_eq!(status(ureq::get(&format!("{base}/mailbox/alice")).call()), 401);
    let post = ureq::post(&format!("{base}/mailbox/alice")).send_string("blob");
